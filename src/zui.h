...
#include <stdio.h>
#include "zui-renderer.h"
#include "zui.h"

i32 main(i32 argc, char **argv) {
    zui_init(renderer, zui_log_stdio, 0);
    while(zui_active()) {
        zui_poll(true);
        zui_window(); {
            zui_label("Hello Zui!");
...
// Server:
#include <stdio.h>
#include "zui-socket-renderer.h"
#include "zui.h"

void renderer(zcmd *cmd, void *user_data) {
    switch(cmd->cmd.id) {
        case ZCMD_REG_FONT:
        case ZCMD_GLYPH_SZ:
        case ZCMD_GET_CLIPBOARD:
            ...
            zui_push(recv(cmd));
            break;
        case ZCMD_POLL_EVENT:
            while(zcmd *c = recv(cmd)) {
                zui_push(c);
            }
            break;
        default:
...
// #include "zui-renderer.h"
i32 main(i32 argc, char **argv) {
    zui_client_init(renderer, send, recv, 0);
}
*/
